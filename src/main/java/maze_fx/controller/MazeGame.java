package maze_fx.controller;

import java.util.ArrayList;
import maze_fx.Sounds;
import maze_fx.models.Field;
import maze_fx.models.FieldType;
import maze_fx.models.Maze;
import maze_fx.observer.AbstractSubject;
import shared.keybordListener.Direction;
import shared.keybordListener.DirectionObserver;
import shared.JOptionQueries;

/**
 * the logic/rules of the game without calculation
 */
public class MazeGame extends AbstractSubject implements DirectionObserver, Runnable {
    ...
    public MazeGame() {
        ...
        this.player = new Field(0, 0, FieldType.PLAYER);
        sound = new Sounds();
        reset();
    }
     ...
     */
    private void moveIt(Direction nextMove) {
        int x = player.getX();
        int y = player.getY();
        switch (nextMove) {
            case Up: {
        ...
        if (x >= 0 && y >= 0 && x < m.getLength() && y < m.getHeigth()) {
            ...
            if (f.isFieldTyp(FieldType.WAY) || f.isFieldTyp(FieldType.START)) {
                player.setPosition(x, y);
            } else if (f.isFieldTyp(FieldType.GOAL)) {//if the player reches the goals
                player.setPosition(x, y);
                sound.win();
    ...
    @Override
    public void onStateChange(Direction direction) {
        move(direction);
    }
