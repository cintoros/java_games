package old.games_swing.Games.g2048.autosolver;
...
public class GameRules {
    ...
    public Direction simulate(int grid[][]) {
        switch (simulateStep1(ArrayCopy.clone2DArray(grid))) {
            case 0:
    ...
    private int simulateStep1(int grid[][]) {
        ...
        for (int richtung = 0; richtung < 4; richtung++) {//für alle freien Felder
            if (richtung == 0) {//werden für alle möglichen Richtungen die Ergebnisse simuliert
                resGrid = right(ArrayCopy.clone2DArray(grid));
            } else if (richtung == 1) {
                resGrid = left(ArrayCopy.clone2DArray(grid));
            } else if (richtung == 2) {
                resGrid = up(ArrayCopy.clone2DArray(grid));
            } else {
                resGrid = down(ArrayCopy.clone2DArray(grid));
            }
            double Resultat;//wenn etwas passiert ist
            if (gridChanged(resGrid, grid)) {//soll er auszählen was es bringt
                Resultat = getFreeGridsCount(resGrid);
                Resultat += score;
                zaehler = 0;
                Resultat += calculatingAdditionalPoints(ArrayCopy.clone2DArray(resGrid));
    ...
    private double calculatingAdditionalPoints(int grid[][]) {
        double Resultat = 0;
        for (int i = 1; i <= 4; i++) {
     ...
     */
    public int getFreeGridsCount(int grid[][]) {
    ...
    private double simulateFutureMove(int egrid[][]) {//simuliert zukünftige Schritte
        ...
        int anzahlFreieFelder = getFreeGridsCount(egrid);
        ...
        for (int bin = 0; bin < 2; bin++) {
            ...
            for (int i = 0; i < anzahlFreieFelder; i++) {
                int[][] testGrid = ArrayCopy.clone2DArray(egrid);
                ...
                for (int richtung = 0; richtung < 4; richtung++) {
                    if (richtung == 0) {
                        resGrid = right(ArrayCopy.clone2DArray(testGrid));
                    } else if (richtung == 1) {
                        resGrid = left(ArrayCopy.clone2DArray(testGrid));
                    } else if (richtung == 2) {
                        resGrid = up(ArrayCopy.clone2DArray(testGrid));
                    } else {
                        resGrid = down(ArrayCopy.clone2DArray(testGrid));
                    }
                    double Resultat = 0;
                    if (gridChanged(resGrid, testGrid)) {//wenn sich was geändert hat
                        Resultat += score;//simuliere weiter und speichere das Resultat
                        Resultat += getFreeGridsCount(resGrid) * (multi);
                        Resultat += calculatingAdditionalPoints(ArrayCopy.clone2DArray(resGrid)) / Math.pow(2, zaehler);
                        Resultat += simulateFutureMove(resGrid);
                        zaehler--;
